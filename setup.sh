...
# 6. Execute o script através do comando: sudo su; ./install 5.9.6
...
then
  ...
  if ! sudo apt install --yes software-properties-common
  then
  ...
  echo "####################################"
  echo "####################################"
  echo " * desconecte da rede em 30 segundos"
  echo "####################################"
  echo "####################################"

  sleep 30

  [ ! -f '/root/install_developers-master/qt-opensource-linux-x64-5.9.3.run' ] && wget https://download.qt.io/archive/qt/${QT_VERSION_MAJOR}/${QT_VERSION}/qt-opensource-linux-x64-${QT_VERSION}.run

  chmod +x qt-opensource-linux-x64-${QT_VERSION}.run

  [ ! -f '/root/install_developers-master/qt-noninteractive.qs' ] && wget https://raw.githubusercontent.com/alfredocoj/install_developers/master/qt-noninteractive.qs

  ./qt-opensource-linux-x64-${QT_VERSION}.run --script qt-noninteractive.qs  #-platform minimal

  echo "####################################"
  echo "####################################"
  echo " * conecte da rede em 30 segundos"
  sleep 30
  echo "####################################"
  echo "####################################"
